            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {accessGranted(PermissionsIDs.HISTORY_STRUCTURAL, Access.READ) && (
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      createShortUuidUrl(`/data-riwayat/pelatihan-struktural/detail/pegawai`, item?.user_id)
                    )
                  }
        ...
        {accessGranted(PermissionsIDs.HISTORY_STRUCTURAL, Access.UPDATE) && (
          <Button
            ...
            onClick={() => {
              const numericId = extractIdFromShortUuidUrl(router?.query)
              if (numericId) {
                router.push(createShortUuidUrl(`/data-riwayat/pelatihan-struktural/edit`, numericId))
              }
            }}
          />
        )}
